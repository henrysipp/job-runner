...
extension ComplexJobRunnerTests {
  ...
  @Test func testJobsReceiveCorrectContext() async throws {
    let counter = TestCounter()
    let context = TestContext(counter: counter)

    let runner = JobRunner<TestContext>(context: context, maxConcurrent: 1, maxAttempts: 3)
  ...
  @Test func testMultipleJobsShareSameContext() async throws {
    let counter = TestCounter()
    let context = TestContext(counter: counter)

    let runner = JobRunner<TestContext>(context: context, maxConcurrent: 2, maxAttempts: 3)
    try await runner.register(CounterJob.self)
    try await runner.start()
...
extension ComplexJobRunnerTests {
  ...
  @Test func testMultipleRunnersWithDifferentContexts() async throws {
    ...
    let runner1 = JobRunner<TestContext>(context: context1, maxConcurrent: 1, maxAttempts: 3)
    let runner2 = JobRunner<TestContext>(context: context2, maxConcurrent: 1, maxAttempts: 3)
